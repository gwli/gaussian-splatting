...
#     MODE   : 2x | enhance     (default enhance)
#     ENGINE : ffmpeg | realesrgan (default ffmpeg)
#     SECS   : trim source to N seconds (default full; set for quick runs)
#     SS     : start offset (default 0)
#   env passthrough: CAP_W IFOV SCALE TILE FP16 CRF NVENC MODEL
#
# Pipeline:  probe -> standardize(stitch/passthrough) -> enhance -> inject360 -> validate
set -e
ROOT=/raid/git/gaussian-splatting
IN="$1"; OUT="$2"; MODE="${3:-enhance}"; ENGINE="${4:-ffmpeg}"; SECS="${5:-}"; SS="${6:-0}"
[ -z "$IN" ] && { echo "usage: run_sr.sh <input> <out.mp4> [2x|enhance] [ffmpeg|realesrgan] [secs] [ss]"; exit 1; }
...
echo "in=$IN  mode=$MODE  engine=$ENGINE  secs=${SECS:-full}"
