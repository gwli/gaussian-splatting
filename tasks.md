...
## Priority 5 — 选帧算法（T-G）
- ☑ **T-G1** 选帧策略三方对比 —— **FPS(位姿最远点)最优,误差自适应次之,均匀最差**。
  公平设计:一个 270-pano 候选池 + 单次 VGGT(共享位姿),三种策略各选**相同 78
  训练帧**,全部在**相同固定 12 测试帧**上评测,隔离选帧算法本身(无 re-stitch/
  re-VGGT 混淆)。后端 fused T-F8。`p3_pano/select_compare.py`:
  | 策略 | held-out PSNR | SSIM | 选+训时间 |
  |---|---|---|---|
  | uniform（均匀时间,现管线) | 18.25 | 0.607 | 70s |
  | **fps（位姿最远点采样)** | **19.38** | **0.623** | 67s |
  | adaptive（误差贪心主动学习) | 18.86 | 0.622 | 146s |
  **发现**:① **FPS 比均匀 +1.13 dB**——位姿最远点采样最大化视角分散,覆盖最好,
  对新视角泛化最强,且零额外成本(只是换选帧)。② **误差自适应比均匀 +0.61 dB 但
  输给 FPS,且贵 2×**(贪心需多轮重训)。③ **诚实洞察**:对 held-out 新视角,
