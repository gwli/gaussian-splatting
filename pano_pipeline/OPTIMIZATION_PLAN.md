...
## 八、选帧算法（T-G,2026-06-15）

把"均匀时间采样 90 帧"换成更聪明的选帧能否提升质量?三方对比(公平设计:一个
270-pano 候选池 + 单次 VGGT 共享位姿,各选**相同 78 训练帧**,**相同固定 12 测试帧**
评测,隔离选帧算法;后端 fused T-F8。`p3_pano/select_compare.py`):

| 策略 | held-out PSNR | SSIM | 选+训时间 |
|---|---|---|---|
| uniform（均匀时间,现管线) | 18.25 | 0.607 | 70s |
| **fps（位姿最远点采样)** | **19.38** | **0.623** | 67s |
| adaptive（误差贪心主动学习) | 18.86 | 0.622 | 146s |

> **① FPS 比均匀 +1.13 dB**:在相机位姿上做最远点采样,最大化视角分散度,新视角覆盖
> 最好——**零额外成本(只换选帧逻辑),白赚 ~1 dB**。
> **② 误差自适应 +0.61 dB 但输给 FPS,且贵 2×**(贪心需多轮重训)。
> **③ 诚实洞察**:对 held-out 新视角,**视角覆盖(FPS) > 误差追逐(adaptive)**——误差
> 贪心会把帧预算过度堆在本质上就难的区域(运动模糊/天空/遮挡),反而牺牲整体覆盖。
> **④ 改进方向**:FPS 种子 + 误差驱动(覆盖×误差加权)可能两者兼得。
> **落地建议**:`prep_pano.sh` 的均匀采样换成位姿 FPS(需先有粗位姿:GPS 或一遍快速
> VGGT),即可在不加帧、不加算力的前提下提升 ~1 dB。

### FPS 接入生产 + FPS×误差混合(T-G2)
**(a) 生产集成**:`prep_pano.sh <S> <insv> <N> fps [POOLX]` 新增 FPS 模式——stitch
POOLX×N 候选池 → 1-crop 粗 VGGT → `fps_select.py` 位姿最远点选 N → 正常裁剪/重训。

**(b) FPS×误差混合**:`select_compare.py` 加 `hybrid`(FPS 种子保覆盖 + 余量按
误差×新颖度贪心)。4-way(同 12 测试 / 78 预算,fused T-F8):
...
> **诚实结论**:**纯 FPS 仍最优**。hybrid 比 uniform/adaptive 好,但**赢不了纯 FPS**;
> adaptive **不稳定**(跨轮 18.86→18.16,会跌破 uniform)。即在本数据上**加误差引导
> 没有帮助、甚至有害**——高误差区是本质难(运动模糊/天空/遮挡),非欠采样,补视角救不回。
> **最终建议:用纯位姿 FPS,不要加误差项。** 这也是个有用的负面教训:对新视角泛化,
> 几何覆盖均匀 > 哪里差补哪里。
