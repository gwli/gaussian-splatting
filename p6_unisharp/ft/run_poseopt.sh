...
BASE="INIT_PLY=$C SKY_FREEZE_R=2.0 LR_SCALE_POS=0.4 ABSGRAD=1 REFINE_STOP_FRAC=0 RESET_EVERY=999999 DEPTH_DIR=p6_unisharp/ft/depth021 DEPTH_NORM=0.25 TEST_IDX_FILE=p3_pano/fair021_test_idx.txt"
# 15k rather than 7k: each camera is seen ~18 times in 7k iterations, too few updates to
# move a pose by the ~1.4 m the diagnostic says is needed. LR_T raised for the same reason.
echo "=== A': depth refine 15k, NO pose opt (iteration control) ==="
env $BASE python p3_pano/train_pano_gsplat_sph.py p3_pano/fair021_d2.json p6_unisharp/ft/runs/gg_p0_021 15000 4096 2>&1 | grep -E "^\[depth\]|EVAL|POSE"
echo "=== B: same + in-training pose optimisation ==="
env $BASE POSE_OPT=1 POSE_GAUGE=1 POSE_START=500 POSE_LR_T=0.1 POSE_LR_R=1e-3 \
  python p3_pano/train_pano_gsplat_sph.py p3_pano/fair021_d2.json p6_unisharp/ft/runs/gg_p1_021 15000 4096 2>&1 | grep -E "^\[depth\]|EVAL|POSE|pose-opt"
echo "=== interpolation self-check + corrected held-out poses ==="
...
echo "=== A' vs B on ORIGINAL held-out poses (shows the frame mismatch) ==="
