...
DOF = sys.argv[6] if len(sys.argv) > 6 else "both"   # both | rot | trans
H = W // 2; dev = "cuda"
...
for c in test:
    ...
    groups = []
    if DOF in ("both", "rot"):   groups.append({"params": [dr], "lr": 3e-4})
    if DOF in ("both", "trans"): groups.append({"params": [dt], "lr": 3e-2})
    ...
    def render():
        ...
        return img.clamp(0, 1).permute(2, 0, 1)
    with torch.no_grad():
