...
echo "=== C: same, but the pose prior actually binds (effective sigma_t ~0.6 m, lr/5) ==="
env INIT_PLY=$C SKY_FREEZE_R=2.0 LR_SCALE_POS=0.4 ABSGRAD=1 REFINE_STOP_FRAC=0 RESET_EVERY=999999 \
    ...
    POSE_OPT=1 POSE_GAUGE=1 POSE_START=500 POSE_LR_T=0.02 POSE_REG=1.0 POSE_LR_R=1e-3 \
  python p3_pano/train_pano_gsplat_sph.py p3_pano/fair021_d2.json p6_unisharp/ft/runs/gg_p2_021 15000 4096 2>&1 | grep -vE "^W07"
echo "=== interpolation self-check ==="
python p6_unisharp/ft/apply_pose_deltas.py p6_unisharp/ft/runs/gg_p2_021/point_cloud/iteration_15000/poses.npz \
  p3_pano/fair021_d2.json p3_pano/fair021_d2_posefix2.json --selfcheck
...
python p6_unisharp/ft/eval_res.py p3_pano/fair021_d2.json p3_pano/fair021_test_idx.txt 4096 1024 \
  ...
  p6_unisharp/ft/runs/gg_p2_021/point_cloud/iteration_15000/point_cloud.ply
echo "=== B on INTERPOLATED-corrected held-out poses (the legitimate number) ==="
python p6_unisharp/ft/eval_res.py p3_pano/fair021_d2_posefix2.json p3_pano/fair021_test_idx.txt 4096 1024 \
  p6_unisharp/ft/runs/gg_p2_021/point_cloud/iteration_15000/point_cloud.ply
echo POSEOPT_B_DONE
