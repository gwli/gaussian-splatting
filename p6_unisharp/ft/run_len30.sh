...
cd /raid/git/gaussian-splatting
# Checkpoints from one long run are NOT equivalent to stopping early: the depth-weight
# anneal and the SH-degree ramp are both indexed by step/ITERS, so a 7k checkpoint of a
# 30k run is a different model than a 7k run (20.306 vs 23.664 on 021). Each length must
# therefore be its own run. 021 was still climbing at 30k, so compare 7k vs 30k per scene.
for S in 023 027; do
  ...
  echo "=== $S: refine 30k (own run, not a checkpoint) ==="
  env INIT_PLY=$C SKY_FREEZE_R=2.0 LR_SCALE_POS=0.4 ABSGRAD=1 REFINE_STOP_FRAC=0 RESET_EVERY=999999 \
    ...
    python p3_pano/train_pano_gsplat_sph.py p3_pano/fair${S}_d2.json p6_unisharp/ft/runs/gg_l30$S 30000 4096 2>&1 | grep -E "^\[depth\]|EVAL|Traceback|Error"
  echo "=== $S: 7k vs 30k ==="
